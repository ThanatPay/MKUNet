...
def Attention(x, emb_dim, heads=8, dim_head=64, dropout=0.):
    ...
    scale = dim_head**(-0.5)
    qkv = k.layers.Dense(inner_dim * 3)(x) # (2,65,3072)
...
def Transformer(x, token, x_AfterPatchEmbedding, num_batch, num_patch, emb_dim, depth, heads, dim_head, mlp_dim, dropout=0.):
    x_AfterConv = tf.reshape(x_AfterPatchEmbedding, (num_batch*num_patch, int(emb_dim**0.5), int(emb_dim**0.5), 1)) # (130,32,32,1)
    x_AfterConv = k.layers.Conv2D(filters=1, kernel_size=3, padding= 'same', strides=1, use_bias=False)(x_AfterConv) # (130,32,32,1)
...
def MSAC_DBUNet(INPUT_SHAPE, OUTPUT_CHANNELS):
    ...
    return tf.keras.Model(inputs=input_layer, outputs=output, name='MSAC_DBUNet')

# if __name__ == "__main__":
#     input_shape = (512, 512, 3)
#     OUTPUT_CHANNELS = 1
#     model = MSAC_DBUNet(input_shape, OUTPUT_CHANNELS)
