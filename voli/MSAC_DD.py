...
def multi_conv_block(x, channels, kernel_size=[3,3,3], strides=(1, 1), padding='same', dilation_rate=[1,1,1],
               is_bn=True, is_relu=True, n=2):
    """ Custom function for conv2d:
        Apply  3*3 convolutions with BN and relu.
    """
    for i in range(1, n + 1):
      x1 = k.layers.Conv2D(filters=channels//3, kernel_size=kernel_size[0],
                          padding=padding, strides=strides, dilation_rate=dilation_rate[0],
                          kernel_regularizer=k.regularizers.l2(1e-4),
                          kernel_initializer=k.initializers.he_normal(seed=5))(x)
      x2 = k.layers.Conv2D(filters=channels//3, kernel_size=kernel_size[1],
                          padding=padding, strides=strides, dilation_rate=dilation_rate[1],
                          kernel_regularizer=k.regularizers.l2(1e-4),
                          kernel_initializer=k.initializers.he_normal(seed=5))(x)
      x3 = k.layers.Conv2D(filters=channels//3, kernel_size=kernel_size[2],
                          padding=padding, strides=strides, dilation_rate=dilation_rate[2],
                          kernel_regularizer=k.regularizers.l2(1e-4),
                          kernel_initializer=k.initializers.he_normal(seed=5))(x)
      x = k.layers.concatenate([x1, x2, x3], axis=-1)
      if is_bn:
          x = k.layers.BatchNormalization()(x)
      if is_relu:
          x = k.activations.relu(x)
...
def FeedForward(x, token, x_AfterPatchEmbedding, num_batch, num_patch, emb_dim, depth, dropout=0.):
    x_AfterConv = tf.reshape(x_AfterPatchEmbedding, (num_batch*num_patch, int(emb_dim**0.5), int(emb_dim**0.5), 1))
    x_AfterConv = k.layers.Conv2D(filters=1, kernel_size=3, padding= 'same', strides=1, use_bias=False)(x_AfterConv)
    x_AfterConv = k.layers.BatchNormalization()(x_AfterConv)
    x_AfterConv = k.activations.relu(x_AfterConv) # (128,32,32,1)

    # AMDA
    a = tf.reshape(x_AfterConv, (num_batch, num_patch, 1, emb_dim)) # (2,64,1,1024)
    b = tf.keras.layers.GlobalAveragePooling2D(data_format='channels_first')(a) # (2,64)
    ...
    for _ in range(depth):
        ...
        ff_x = k.layers.Dense(emb_dim/16)(ff_x)
        ff_x = gelu(ff_x)
...
class encoder(tf.keras.Model):
    def __init__(self, img_size, patch_size, emb_dim, depth):
        super().__init__(img_size, patch_size, emb_dim, depth)
    ...
    def __call__(self, x):
        ...
        pos_embeddings = Pos_embedding(img_size=self.img_size, patch_size=self.patch_size, emb_dim=self.emb_dim)(x) # (2,65,1024)
        cls_tokens = Cls_token(img_size=self.img_size, patch_size=self.patch_size, emb_dim=self.emb_dim)(x) # (2,1,1024)
        ...
        vit_layerInfo = []
        for i in range(4):
            x_vit = FeedForward(x_vit, cls_tokens, x_AfterPatchEmbedding, b, n, self.emb_dim, self.depth)
...
def MSAC_DD(INPUT_SHAPE, OUTPUT_CHANNELS):
    filters = [64, 128, 256, 512]

    input_layer = k.layers.Input(shape=INPUT_SHAPE, batch_size=2, name="input_layer")  # (2,512,512,3)
    vit_layerInfo = encoder(img_size=(512,512), patch_size=(64,64), emb_dim=1024, depth=1)(input_layer)
    ...
    e2 = multi_conv_block(e2, filters[1], kernel_size=[3,5,7], dilation_rate=[1,1,1], n=1)
    e2 = conv_block(e2, filters[1], n=1)  # (2,256,256,128)
    # e2 = k.layers.Dropout(0.1)(e2)

    # block 3
    e3 = k.layers.MaxPool2D(pool_size=(2, 2))(e2)  # (2,128,128,128)
    e3 = multi_conv_block(e3, filters[2], kernel_size=[2,4,6], dilation_rate=[1,1,1], n=1)
    e3 = conv_block(e3, filters[2], n=1)  # (2,128,128,256)
    # e3 = k.layers.Dropout(0.1)(e3)

    # block 4
    e4 = k.layers.MaxPool2D(pool_size=(2, 2))(e3)  # (2,64,64,256)
    e4 = multi_conv_block(e4, filters[3], kernel_size=[1,3,5], dilation_rate=[1,1,1], n=1)
    e4 = conv_block(e4, filters[3], n=1)  # (2,64,64,512)
    ...
    d2 = k.layers.concatenate([d3, e3, v3], axis=-1) # (2,128,128,577)
    ...
    d1 = k.layers.concatenate([d2, e2, v2], axis=-1) # (2,256,256,321)
    ...
    d = k.layers.concatenate([d1, e1, v1], axis=-1) # (2,512,512,193)
    ...
    return tf.keras.Model(inputs=input_layer, outputs=output, name='MSAC_DD')


# if __name__ == "__main__":
#     input_shape = (512, 512, 3)
#     OUTPUT_CHANNELS = 1
#     model = MSAC_DD(input_shape, OUTPUT_CHANNELS)
